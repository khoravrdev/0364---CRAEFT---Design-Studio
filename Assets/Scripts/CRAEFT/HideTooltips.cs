...
public class HideTooltips : MonoBehaviour
{
    void Start()
    {
        ...
        // Query all Label elements with the name "tooltip" and hide them
        var tooltipLabels = root.Query<Label>(name: "tooltip").ToList();
    ...
    {
        // Query for any Label inside the VisualElement that has the name "tooltip"
        var tooltipLabels = element.Query<Label>(name: "tooltip").ToList();
